...
{\footnotesize 38000 Grenoble Cedex}                                        & {\bf Tuteur de l'école}\\
{\footnotesize ~~}                          & ~~~Mounié Grégory\\
\end{tabular}
\end{center}
\newpage
...
~~ Une monnaie locale complémentaire(\textbf{MLC}) est un titre de paiement qui circule dans une zone géographique donnée. \`{A} Grenoble, il s'agit du Schéma de Cohérence Territorial(\textbf{SCoT}). Les professionnels du réseau doivent respecter la Charte des valeurs, pierre fondatrice de l'Association. Les éléments principaux de cette charte sont les suivants :
\begin{itemize}
...
\end{itemize}
Elle est encadrée par la loi Hamon sur l'économie sociale et solidaire promulguée le 31 juillet 2014. \\
...
Pour avoir l'autorisation d'en échanger, il faut être adhérent à l'Association. 

\paragraph{Dynamique} \label{par:dynamique}
L'engouement lié aux monnaies locales complémentaires ne cesse de croître en France, en témoigne leur nombre, et une législation qui les intègre de plus en plus. Il est donc nécessaire et profitable de créer une dynamique de mutualisation, de coopération et de réflexion commune en partageant les richesses de chacune. C'est l'objectif que s'est donné le "mouvement SOL", association née d'initiatives citoyennes sur différents territoires. Ainsi, elle cherche à communiquer, redistribuer, former sur les différents aspects de la gestion d'une monnaie, dont les outils numériques font partie, et notamment la mise en place d'une monnaie dématérialisée commune, tout en permettant à chaque monnaie locale de garder sa propre politique de gestion.
...
\item les comités décisionnels : 
	...
	\end{itemize}
\item les groupes locaux : collectif d'adhérents d'un regroupement de villes appartenant au territoire du Cairn. Il se fait le relais de l'Association sur le terrain. Chaque groupe local prend notamment la décision finale d'intégrer ou non un professionnel dans le réseau.
...
\item conscience écologique : une consommation locale issue d'une production locale réduit en principe les coûts liés aux transports, et donc impacte la consommation énergétique. C'est aussi amener à réfléchir à la consommation/production d'un point de vue systémique : en plus du produit fini en boutique, tout un processus de production est mis en place, et doit être considéré.
\end{itemize}
...
\end{itemize} 
Les sources de financement de la Pousada étant bien supérieures à celles du CAIRN (financement par les entreprises intéressées), une telle contribution pourrait soulager l'Association d'une partie de la licence du logiciel support.
...
Enfin, l'outil Cyclos est utilisé par plusieurs monnaies locales bien établies dans leur région (SoNantes à Nantes, Eusko au Pays-Basque, Bristol Pound à Bristol), indicateur de confiance pour l'Association ainsi que les autorités juridiques chargées d'encadrer les monnaies locales. La perspective de mutualisation des technologies à l'échelle du mouvement SOL (association coordinatrice de monnaies locales en France) est un argument de plus en faveur de Cyclos.
 ...
 \newpage
\paragraph{Architecture logicielle} \label{par:archi_cyclos}~
\begin{figure}[h!]
    \centering
...
\paragraph{Les services web} \label{par:services_web}
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.49]{Images/service_web.png}
    \caption{Cette figure est un schéma descriptif de l'utilisation d'un service web}
...
\paragraph{Le design pattern Modèle-Vue-Contrôleur}  \label{par: mvc}~ \\
...
\subsubsection{Les couches métier}\label{subsubsec:application}
Le code de l'application est séparé en deux bundles distincts. L'objectif d'un bundle (voir \ref{subsubsec:interet}) est d'implémenter une fonctionnalité bien précise. \\

\textbf{Le bundle CyclosBundle} ~\\
~~ Le premier bundle se nomme \textit{CyclosBundle} et contient toute la logique liée à la configuration interne de Cyclos(cf. Fig\ref{fig:cyclos_config}). Il s'agit, pour la très grande majorité des actions, de méthodes CRUD(Create, Read, Update, Delete) sur les différentes ressources du système(Réseau, Groupe, Utilisateur, Type de compte, Type de transfert, ...). Son objectif principal est l'automatisation de la mise en place de leur configuration, très complexe à mettre en place "à la main" (via le canal \textit{web principal}) et nécessitant donc une bonne maîtrise de l'outil. 
Ce bundle est indépendant de tout bundle externe. \\

Dans le design pattern MVC(voir \ref{par: mvc}), le rôle de la couche métier(ou modèle) est l'interaction avec la base de données. Pour ce bundle, il s'agit de la base de données Cyclos. Cette couche est séparée en deux types de classes, toutes des objets PHP mais situés dans des dossiers distincts car ayant des rôles distincts : les managers et les services. \\
Les managers contiennent les procédures modifiant la configuration de Cyclos. Les procédures de lecture de données Cyclos sont, quant à elles, définies dans des services car elles doivent être accessibles depuis les autres bundles : UserBundle dans notre cas.
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.5]{Images/cyclosbundle_metier.png}
    \caption{Cette figure montre l'agencement de la couche métier du bundle \textit{CyclosBundle} pour la ressource : type de compte}
    \label{fig:cyclosbundle_metier}
\end{figure}
~\\
Chacune de ces classes permet d'interagir avec la base de données par le biais des services web(voir \ref{par:services_web}) grâce aux classes \verb!Cyclos\xxxService! présentes en attribut.
...
Le second bundle se nomme \textit{UserBundle} et contient toute la logique métier liée à la gestion des utilisateurs de l'application web ainsi que la gestion des échanges d'unités de comptes. Il dépend donc fortement du bundle CyclosBundle. \\ \\
L'approche concernant la couche métier de ce bundle diffère de celle mise en place pour le bundle \textit{CyclosBundle}. 
Symfony propose des services de gestion de base de données nommés ORM(Object-Relation Mapper). L'intérêt d'un ORM est qu'il permet de changer de paradigme de raisonnement : on utilise des entités(objets gérés par l'ORM) et on pense Programmation Orientée Objet, et non gestion de base de données. \\ \\
\underline{Exemple}: le diagramme UML ci-dessous(Fig. \ref{fig:uml})  montre la relation OneToOne unidirectionnelle entre les entités \textit{User} et \textit{Card}. Imaginons qu'on souhaite récupérer la carte d'une instance \$user de \textit{User}.\\
sans ORM : \verb!SELECT User,Card FROM User JOIN Card WHERE id=$user.id ! \\
avec un ORM : \verb!$user->getCard()! \\

Même si l'écriture est différente et bien plus lisible avec l'utilisation d'un ORM, car on n'écrit plus des requêtes mais des fonctions, il faut être conscient que ce sont bien des requêtes SQL qui sont produites par le service d'ORM pour accéder à une entité dans la base de données Symfony. Un usage abusif de ces fonctions pourrait donc avoir un impact négatif sur les performances de l'application, car le nombre de requêtes effectuées doit être minimisé.\\
On peut faire les associations suivantes d'interprétation par un ORM :
\begin{itemize}
\item définition d'une classe Entité \textit{A} : une table en base de données
\item une instance de \textit{A}(une entité) : une entrée dans la table
\item un attribut de \textit{A} : un champ dans le table
\item une relation entre \textit{A} et une classe Entité \textit{B} : un champ dans la table contenant une clé étrangère référençant l'entité de classe \textit{B} liée.
\end{itemize}
...
\begin{figure}[h!]
    ...
    \label{fig:uml}
\end{figure}
Dans une relation entre entités, l'entité dite \textit{propriétaire} est celle qui contient la référence(clé étrangère) vers l'entité dite \textit{inverse}. Il s'agit d'une information importante pour la gestion des données, notamment pour la suppression d'entités. En effet, on ne peut supprimer une entité(entrée dans la table) que si les entités liées par une relation(champs contenant des clés étrangères) ont été supprimées avant, pour éviter une \textbf{cassure} de la base de données.\\
...
En plus des données classiques du membre, deux attributs pour comptabiliser le nombre d'essais de saisie de mot de passe ou de la carte de sécurité ont été ajoutés : passwordTries et cardKeyTries. Ainsi, au bout de trois essais infructueux, l'espace membre est bloqué. \\
...
Cette question s'est posée après réalisation d'un manquement à une spécification : toute transaction doit contenir un intitulé(court) et un motif long(voir \ref{subsec:probleme}). Hors, les classes Cyclos contenant les données des transactions ne sont munies que d'un attribut "description" ne permettant pas de distinguer le motif de l'intitulé. Il faudrait donc, pour respecter la spécification, créer une entité \textit{Transaction}, la lier à l'objet Cyclos équivalent par un identifiant unique(à l'image de l'attribut \textit{cyclosID} de la classe \textit{User}), et rajouter à cette entité des attributs "motif" et "intitulé". \\ \\
Cependant, une difficulté rencontrée n'a pas permis de réaliser cette entité. Cette difficulté concerne la maîtrise de la gestion des données  liées aux transactions dans Cyclos. Elle repose sur deux caractéristiques : la complexité du système et une documentation trompeuse(un attribut manquant et une nomenclature qui prête à confusion). Dans le temps imparti, cette correction n'a donc pas pu être achevée.

\subsection{Fonctionnement de l'application} \label{subsec:fonctionnement}
~~ A partir de l'analyse effectuée dans les sous-sections précédentes(\ref{subsec:cyclos} et \ref{subsec:symfony}), un workflow simplifié du fonctionnement de l'application web peut être présenté comme suit.
\subsubsection{Architecture globale et enjeux associés}\label{subsubsec:archi_global}
\begin{figure}[h!]
    \centering
...
\paragraph{La connexion réseau} \label{par:connexion_reseau}
...
\paragraph{le fichier de correspondances} \label{fichier_correspondances}
...
\textbf{Inconvénient} Une telle décision signifie également que, entre deux phases de régularisation, les fonds de garantie sont déséquilibrés : certains cairns numériques(resp. billets) sont gagés par le fonds de garantie billets(resp. numériques). \\
\underline{Illustration du problème} : Entre deux phases de régularisation, le fonds de garantie numérique(resp. billets) a un solde de 3000 euros au lieu de 4000(resp.6000 au lieu de 5000). Un professionnel demande à reconvertir 3500 cairns, qu'il a donc sur son compte cairn numérique. Il ne le pourra pas jusqu'à la prochaine régularisation.  La périodicité dépendra donc de l'activité des professionnels.\\
Ces choix devront faire l'objet d'une expertise et d'une validation de l'ACPR. Cependant, le document (\cite{exemption}) stipule qu'en dessous du million d'euros de chiffre d'affaire, il y a exemption d'agrément. Cela laisse donc le temps de se former sur cette future expertise et d'adapter la solution en fonction.
Dans l'état actuel, l'application gère les transferts d'unités de compte cairns sur la plateforme Cyclos, mais ne traite pas les virements bancaires.
...
\subsection{L'authentification} \label{subsec:authentification}
...
\paragraph{Authentification à Cyclos} \label{par:authentification_cyclos}
Pour rappel, l'application accède au logiciel Cyclos par le biais des services web (voir \ref{par:connexion_reseau}). Pour ce faire, une méthode d'authentification est également requise lors de l'étape de "connexion réseau". Il en existe deux modes  dits \textit{sans état} : login/password et accès client.
\begin{itemize}
...
\subsubsection{la carte de sécurité Cairn} \label{subsubsec:carte}
...
\subsection{L'autorisation} \label{subsec:autorisation}
...
L'application ne doit à aucun moment retourner une erreur au visiteur, révélateur d'un cas non géré lors du développement et donc d'une potentielle faille exploitable. Cette sous-section a donc pour but de passer en revue les failles de sécurité et la façon dont elles sont bloquées. Deux types d'actifs sont susceptibles d'être impliqués par dans une faille de sécurité.
\paragraph{Les actifs primordiaux} ~~ \\
Il s'agit de l'ensemble des processus métiers de l'application et ses informations. 
\begin{itemize}
\item \textbf{les routes} Point d'entrée de l'application, le fichier de correspondances répertorie l'ensemble des routes valides. Ces routes peuvent contenir des paramètres dynamiques - token, identifiant, mot, ...- dont les valeurs sont spécifiées par le visiteur. Une règle d'or lors du développement est "\textit{Never trust user input}" . Ainsi, il est nécessaire de s'assurer que les valeurs indiquées correspondent bien au type attendu.\\
\underline{Exemple} : \verb!http://www.monsite.com/example/?id=17546&nom=cairn! \\
...
\item \textbf{les failles XSS}(Cross-site scripting) : Il s'agit d'injecter du code HTML contenant du JavaScript dans les pages pour le faire exécuter aux visiteurs. Pour "injecter" du code, il faut donc que le serveur récupère une donnée fournie par le visiteur malveillant. Cela se fait lors de deux opérations : lors d'une requête HTTP GET(une URL) ou lors d'une requête HTTP POST(formulaire). Le premier cas est décrit ci-dessus. La soumission de formulaires étant gérée par Symfony, le  cas des failles XSS a été traité.
\item \textbf{les injections SQL} De même, en utilisant les outils fournis par Symfony pour effectuer des requêtes en base de données, le cas des injections SQL est géré. 

\end{itemize}

Les injections sont des cas classiques auxquels de nombreux développeurs se sont déjà attaqués. La solution est connue : il faut échapper les caractères des champs de formulaire avant de les traiter. Seulement, il suffirait d'un oubli sur l'un des nombreux champs de formulaire de l'application pour avoir une faille. C'est pourquoi l'automatisation de l'échappement par le framework permet de se concentrer sur la logique métier. \\
...
\item \textbf{attaque CSRF} : La faille CSRF(Cross Site Request Forgeries) consiste à forcer un utilisateur authentifié à exécuter une requête à son insu. Plus cet utilisateur à de droits, plus les dommages peuvent être importants. Elle fait donc appel à la naïveté de la victime, mais aussi à celle de l'application.
...
\section{La phase de tests} \label{sec:tests}
~~ Dans le cadre d'un projet web, les tests se déroulent tout au long du développement. En effet, chaque nouvelle action de contrôleur implémentée est suivie de vérifications "à la main"	permettant de s'assurer des résultats obtenus. Cependant, au fur et à mesure que l'application croît, que les entités sont de plus en plus liées les unes aux autres, l'automatisation des tests permet d'éviter, notamment, que des fonctionnalités précédemment opérationnelles soient par la suite exposées à des erreurs.
\subsection{La phase de pré-tests} \label{subsec:pretests}
~~ Pour écrire des tests efficacement, il faut être en mesure de contrôler le contenu de la base de données initialement et après chaque action réalisée lors d'un test. Comme décrit dans le paragraphe \ref{par:bdd} sur les bases de données, il y a deux bases de données à gérer, et doivent concorder à tout instant. Ainsi, il faut les construire de sorte à être parfaitement anticipables lors des tests. Cependant, manipuler la base de données Cyclos est loin d'être chose aisée, comme en témoigne ce passage tiré de la documentation officielle de Cyclos \cite{cyclos_doc}
\begin{quotation}
\textit{In most cases we would recommend to just start over and manually copy the products, configurations, accounts and transaction types, field etc. you created. For most people this is the best solution. \\ \\
 Only for very advanced users that know a lot about database, ...}
\end{quotation} 
Cela remet en question l'idée de départ qui était de créer un réseau indépendant pour les tests(voir Fig.\ref{fig:cyclos_config}), qui pourrait être copié, réinitialisé etc. mais de conserver une unique base de données Cyclos. \\
Hors il faut, à chaque lancement du script de tests, créer une nouvelle base de données de tests(et donc supprimer l'ancienne), réinstaller la configuration "à la main" de tout le système, insérer une liste d'utilisateurs et de transactions pour avoir un environnement opérationnel. \\ \\
Étant inconcevable qu'un script contienne une partie manuelle, il a été nécessaire d'installer Cyclos une première fois avec toute la configuration, de la sauvegarder dans un fichier de backup au format sql nommé \textit{backup.sql}. Ensuite, le script de pré-tests suit les étapes suivantes :
\begin{enumerate}
\item Supprimer l'ancienne base de données de tests Cyclos
\item Supprimer l'ancienne base de données de tests Symfony
\item Créer une nouvelle base de données de tests Cyclos
\item Créer une nouvelle base de données de tests Symfony
\item Importer \textit{backup.sql}. Cela permet de ne pas avoir à recommencer les étapes de configuration
\item Importer les utilisateurs fournis par \textit{members.csv} via les services web
\item Importer les transactions fournies par \textit{transactions.csv} via les services web
\item Créer les équivalents Symfony de tous les utilisateurs Cyclos et définir des référents pour les professionnels en respectant les rôles tels que décrits dans la partie \ref{subsec:autorisation}
\end{enumerate}
Une fois toutes ces étapes achevées, on obtient deux bases de données initialisées, cohérentes entre elles, avec des utilisateurs ayant des rôles différents et un historique de transactions. 
\paragraph{Import de données}
Cyclos propose une fonctionnalité permettant d'importer des données depuis des tableurs au format csv. Pour cela, il faut écrire un script compréhensible par le logiciel, écrit en langage Groovy(très proche de Java) qu'on appelle à l'aide de la classe PHP \verb!CustomScriptService! et depuis lequel on charge le contenu des fichiers csv. Cela permet au développeur de personnaliser sa liste des utilisateurs et la liste des transactions en fpnction des tests qu'il souhaite réaliser. \\ \\
~~ Si l'étape 8 est lancée immédiatement après l'étape 7, le script plante car elle est lancée alors que l'import des étapes 6 et 7 n'est pas terminé. Pour cette raison, un délai doit être placé entre les deux étapes pour s'assurer que la base de donnée est à jour lors du lancement de l'étape 8. \\ \\
L'idée de cette partie est de montrer que le processus de pré-tests est très fastidieux mais nécessaire. De plus, l'import de la base de donnée(étape 5) prend plusieurs minutes. Hors, un script de tests trop long peut inciter le développeur à ne pas s'en servir. Il serait donc intéressant d'apprendre, à l'avenir, à manipuler la base de données Cyclos à l'aide d'un script SQL, afin de minimiser la durée de la phase de pré-tests. 
\subsection{Les tests fonctionnels} \label{subsec:tests_fonctionnels}
Une fois la phase de pré-tests achevée, l'application doit être testée.
Les tests fonctionnels ont pour objectif de tester les fonctionnalités et de vérifier que l'ajout d'une nouvelle n'altère pas le reste de l'application. Pour écrire un test fonctionnel dans le cadre d'une application web, on a besoin principalement de trois éléments :
\begin{itemize}
\item un client (jouant le rôle du navigateur)
\item une requête HTTP dirigeant vers la page à tester
\item une réponse HTTP pour vérifier que le contenu est bien celui attendu	
\end{itemize}  

\paragraph{Framework de tests} ~\\
Symfony fournit un framework de tests nommé \textit{PHPUnit} permettant de réaliser des tests unitaires et fonctionnels. Il a donc été utilisé dans le projet. \\ 
Ci-dessous un exemple typique de test fonctionnel avec \textit{PHPUnit} pour la fonctionnalité du périmètre fonctionnel "ajout d'un nouveau bénéficiaire" (voir \ref{subsec:probleme} ). Cette opération est considérée comme sensible. On s'attend donc à ce qu'une identification par carte soit demandée au client.
\begin{verbatim}
$client = static::createClient();      //création du client
$crawler = $client->request('GET', '/beneficiary/new'); //envoi de la requête HTTP
$form = $crawler->selectButton('card_submit')->form(); //récupération du formulaire
$form['card[fields]']->setValue('1111'); //remplissage du champ
$crawler = $this->client->submit($form); //soumission du formulaire                               

// vérification du contenu de la page retournée
$this->assertSame(1,$crawler->filter('html:contains("clé valide")')->count()); 
\end{verbatim}
D'après cet exemple, on peut constater qu'il y a deux éléments prépondérants pour réaliser un test fonctionnel. D'une part, le \textit{client} effectue les requêtes HTTP, soumet les formulaires et récupère les réponses. D'autre part, le \textit{crawler} permet de parcourir le code HTML via le DOM(Document Object Model). Il s'agit d'une API utilisée notamment en Javascript pour accéder aux différentes balises du code HTML d'une page web. \\ \\
On comprend dès lors l'intérêt de maîtriser le contenu de la base de données avant la phase de tests(voir \ref{subsec:pretests}). En effet, il faut être en mesure d'anticiper le comportement de l'application en fonction des données pour pouvoir la tester. \\
\underline{Exemple} : En environnement de test, toutes les entrées de toutes les cartes de sécurité valent \verb!1111!. Le cas échéant, comment s'assurer du comportement pour une saisie valide/invalide sachant que les entrées sont générées aléatoirement ? \\ \\ \\

La phase de tests est sans nul doute la partie demandant le plus d'améliorations dans le projet. En effet, la phase de pré-tests est très fastidieuse et les scéniaros de tests écrits se limitent à une seule fonctionnalité, ne combinant pas les séquences d'actions qu'un utilisateur pourrait effectué. L'outil d'intégration continue \textit{Travis CI} a été ajouté au projet. 
...
Ensuite, j'ai pu assimiler de manière très basique les langages d'intégration web, tels HTML, CSS ou encore Javascript et jQuery. Cette compétence n'ayant pas été demandée dans le cadre du stage, quelques connaissances ont malgré tout été nécessaires afin de visualiser les différents écrans et qu'ils contiendraient. \\
Je me suis également plongé plus profondément sur les enjeux de sécurité informatique. \\
...
différentes. Tout cela m'a permis d'ouvrir mon champ de vision et d'envisager de nouvelles perspectives professionnelles, comme avoir le statut d'indépendant et travailler sur des projets web dont les objets sont à but éthique, ou encore me rapprocher de mon domaine de formation en intégrant des équipes de recherche à l'image de l'équipe STEEP(Soutenabilité, Territoires, Environnement, Economie et Politique) de Grenoble .

\newpage

\bibliographystyle{plain}
\bibliography{prerapport}
